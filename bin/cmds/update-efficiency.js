...
exports.handler = async function(argv) {
    ...
    if (argv.offline) {
        ...
        try {
            ...
            spinner = ora(
                `Generating script to update Identity ${chalk.yellow(
                    argv.identity
                )} with an efficiency of ${chalk.yellow(argv.efficiency)}...`
            ).start();

            const filename = generateUpdateEfficiencyScript(
                argv.identity,
                argv.smchainid,
                argv.efficiency,
                argv.sk1,
        ...
        spinner = ora(
            `Updating Identity ${chalk.yellow(argv.identity)} with an efficiency of ${chalk.yellow(argv.efficiency)}...`
        ).start();
