    ...
    if (argv.offline) {
        if (!argv.smchainid) {
            return console.error(
                chalk.red('Server Management Subchain Id needs to be specified (--smchainid) in offline mode')
            );
        }
        ...
        try {
            console.error(
                chalk.bgBlue.bold('  INFO  ') +
                    ' Remember to always verify that the clock of your computer is synced when using the offline mode (see README for the reasons).\n'
            );
            spinner = ora(
                `Generating script to add a cancel coinbase message for height ${chalk.yellow(
                    argv.height
                )} and index ${chalk.yellow(argv.index)} to Identity ${chalk.yellow(argv.identity)}...`
            ).start();

            const filename = generateAddCoinbaseCancelScript(
                argv.identity,
                argv.smchainid,
                argv.height,
                argv.index,
            ...
            spinner.succeed();
            console.error(
                `Execute ${chalk.yellow(filename)} script on a machine with curl command and an Internet connection.`
            );
        } catch (e) {
        ...
        spinner = ora(
            `Adding coinbase cancel message for height ${chalk.yellow(argv.height)} and index ${chalk.yellow(
                argv.index
            )} to Identity ${chalk.yellow(argv.identity)}...`
        ).start();

        try {
            const data = await manager.addCoinbaseCancel(
                argv.identity,
                argv.height,
                argv.index,
                argv.sk1,
            ...
            spinner.succeed();
            console.error(
                `Please wait for the next block to see the effect. Entry hash of the update: ${chalk.yellow(
                    data.entryHash
                )}.`
            );
        } catch (e) {
