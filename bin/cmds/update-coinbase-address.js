...
exports.handler = async function(argv) {
    ...
    if (argv.offline) {
        try {
            ...
            spinner = ora(
                `Generating script to update Identity ${chalk.yellow(
                    argv.identity
                )} with coinbase address ${chalk.yellow(argv.fctaddress)}...`
            ).start();

            const filename = generateUpdateCoinbaseAddressScript(
                argv.identity,
                argv.fctaddress,
                argv.sk1,
                argv.secaddress,
        ...
        spinner = ora(
            `Updating Identity ${chalk.yellow(argv.identity)} with coinbase address address ${chalk.yellow(
                argv.fctaddress
            )}...`
        ).start();
